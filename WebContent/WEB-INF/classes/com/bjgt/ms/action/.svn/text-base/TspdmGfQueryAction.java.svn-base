 ...
 */
@Controller
@RequestMapping(value = "spdmgf")
public class TspdmGfQueryAction extends CrudActionSupport<TspdmGf> {
	@Autowired
	private TspdmGfManager tspdmGfManager;
	...
	public String webquery() {
		...
		try {
			...
			// 设置查询参数
			PageResult<TspdmGf> page = new PageResult<TspdmGf>();
			page.setPageSize(pageSize);
			page.setPageNo(pageno);
			page.setCountNo(countNo);
			page.setP1(querystr);
			try {
				page = tspdmGfManager.getListByPage(page);
			} catch (Exception e) {
				e.printStackTrace();
	 ...
	 * 
	 */
	@RequestMapping(value = "gospdmgflist.html", method = { RequestMethod.POST })
	public String gospdmgflist() {
		try {
			Integer pagesize = Integer.valueOf(request
					.getParameter(Constants.PAGESIZE));
			Integer pageno = Integer.valueOf(request
	...
	public void mwebquery() {
		try {
			...
			List<TspdmGf> list = page.getList();
			JSONArray arr = new JSONArray();
			JSONObject jsonObject = null;
			for (TspdmGf ttmNew : list) {

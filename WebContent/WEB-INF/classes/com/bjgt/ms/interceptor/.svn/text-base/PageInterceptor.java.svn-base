...
public class PageInterceptor implements Interceptor {
	...
	public Object intercept(Invocation invocation) throws Throwable {
		...
		// Page对象获取，“信使”到达拦截器！
		PageResult page = searchPageWithXpath(boundSql.getParameterObject(),
				".", "page", "*/page");
	 ...
	 * 根据给定的xpath查询Page对象
	 */
	private PageResult searchPageWithXpath(Object o, String... xpaths) {
		JXPathContext context = JXPathContext.newContext(o);
		Object result;
		for (String xpath : xpaths) {
			try {
				result = context.selectSingleNode(xpath);
			} catch (JXPathException/* twt修改 */e) {
				continue;
			}
	...
	private MappedStatement copyFromMappedStatement(MappedStatement ms,
		...
		// twt修改
		String[] strings = ms.getKeyProperties();
		if(strings==null||strings.length<=0){
			builder.keyProperty(null);
		}else{
			builder.keyProperty(strings[0]);
		}

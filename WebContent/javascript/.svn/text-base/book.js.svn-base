$(document).ready(function() {
	$("#skip").click(function() {
		if (xyw_checkspace($('#skippage').val())) {
			alert("请输入跳转页码！");
			return false;
		}
		if(isNaN($('#skippage').val())){
			alert("跳转页码只能为数字！");
			return false;
		}
		if($('#skippage').val()>232||$('#skippage').val()<0){
			alert("跳转页码的范围为0~232！");
			return false;
		}
		$("#page").val($("#skippage").val());
		$("#frm").submit();
	});
    var Beacon={}
	//add fav
    ...
    left_bar.click(function() {
        if (current - 2 >= 0) {
            ...
            if (isIE) {
                ...
                flip.find('img').attr('src', '/css/bookcss/flip_l.png');
            }
            flip.find('.container').css('background', 'url(/css/bookcss/book_r.jpg) left top no-repeat');
    ...
    right_bar.click(function() {
        if (current + 2 <= total - 1) {
            ...
            if (isIE) {
                ...
                flip.find('img').attr('src', '/css/bookcss/flip_r.png');
            }
            flip.find('.container').css('background', 'url(/css/bookcss/book_l.jpg) left top no-repeat');

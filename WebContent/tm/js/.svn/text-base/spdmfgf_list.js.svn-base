			...
			if (data.list) {
				if (data.list.length > 0) {
					...
					$.each(data.list, function(index, spdmgf) {
						addother +='<tr class="t_table_tr'+(index %2!=0?2:'')+'" onclick="getdetail(\''+(spdmgf.fspdm ==null ?"":spdmgf.fspdm)+'\',\''+(spdmgf.fgroup ==null ?"0":spdmgf.fgroup.substr(0, 2))+'\');"><td class="t_table_td_bianhao"><nobr> '+((pageno-1)*pagesize+index+1)+' </nobr></td><td class="t_table_td_left"><nobr> '+(spdmgf.fgroup ==null ?"":spdmgf.fgroup)+' </nobr></td><td class="t_table_td_right"><nobr>'+(spdmgf.fspdm ==null ?"":spdmgf.fspdm)+'</nobr></td><td class="t_table_td_right"><nobr>'+(spdmgf.fspname ==null ?"":spdmgf.fspname)+'</nobr></td></tr>';
					});
					// 添加页面
//					 alert(addother);
